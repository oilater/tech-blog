...
import {
  DEFAULTS,
  type ElementType,
  type GetMotionTlProps,
  type PreviousMotionValueType,
} from '../core/types';
...
function getFromTo(
  ...
  if (key === 'color') {
    const from =
      value.from ?? previous[key]?.to ?? getCurrentColor(element);
    const to =
      value.to ?? previous[key]?.from ?? getCurrentColor(element);
    return { from, to };
  }

  const from =
    value.from ?? previous[key]?.to ?? getDefaultValue(key, 'from');
  const to =
    value.to ?? previous[key]?.from ?? getDefaultValue(key, 'to');
